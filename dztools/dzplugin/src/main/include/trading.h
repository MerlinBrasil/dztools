...
} TICK; // multi-quote tick

typedef struct TICK1
{
	long	type;
	float fVal;	
	DATE	time;	// time stamp, GMT
} TICK1; // single-quote tick
...
typedef struct ASSET
{
	...
	int	*pnumTicks;	// list of quote frequencies
	int	flags;
...
#define SAVE		(1<<10)	// save series from here
...
#define SCREENSAVER (1<<22) // allow a screen saver 
